		...
		for (int iMaterial = 0; iMaterial < m_Materials->size(); iMaterial++)
		{
			...
			if (pMaterial->strTexture[0])
			{
				...
				if (!bFound)
				{
					...
					int size = strFile.rfind(L"/");
					pMaterial->strTexture = strFile.substr(0, size + 1) + pMaterial->strTexture;
...
HRESULT CMeshLoader::LoadGeometryFromOBJ(const WCHAR* strFileName)
{
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"mtllib"))
		{
			...
			wstring strFile = strFileName;
			int size = strFile.rfind(L"/");
			strFile = strFile.substr(0, size + 1);
			strMaterialFilename = strFile + strMaterialFilename;
		}
...
HRESULT CMeshLoader::LoadMaterialsFromMTL(const WCHAR* strFileName)
{
	...
	vector<Material*> *temp = MESHMANAGER->GetMaterials(str, m_Materials);
